...
/**
 * Guardian signs an EIP-712 "GuardianDelegation" message off-chain, authorizing
 * `delegate` to submit access approvals on their behalf until `validUntil`.
 */
const signGuardianDelegation = async (
  ...
  if (!signer || typeof (signer as ethers.Signer).signTypedData !== "function") {
    throw new Error("A connected wallet signer is required to delegate guardian approval.");
  }
...
/**
 * Delegate submits an access approval on behalf of `guardian` using a signed
 * EIP-712 GuardianDelegation grant.
 */
const approveAccessByDelegation = async (
  ...
  const contract = ensureWriteContract();
  if (!contractHasFunction(contract, "approveAccessByDelegation(uint256,address,uint256,uint256,bytes,string)")) {
    throw new Error("Current contract does not support guardian delegation.");
  }
